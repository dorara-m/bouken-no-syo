...
const pugFunc = async(isAll) => {
  ...
  data.cmsData = await fetch('https://boken-no-syo.microcms.io/api/v1/blogs', {
    headers: {
      'X-MICROCMS-API-KEY': '5ed4233a933b4f65b45ab6ffe455d1d76730',
    },
  })
    .then((response) => {
      if (!response.ok) {
...
const pugPagesFunc = async() => {
  const cmsData = await fetch('https://boken-no-syo.microcms.io/api/v1/blogs', {
    headers: {
      'X-MICROCMS-API-KEY': '5ed4233a933b4f65b45ab6ffe455d1d76730',
    },
  })
...
function watch() {
  ...
  gulp.watch('src/pug/_pages.pug', pugPagesFunc)
